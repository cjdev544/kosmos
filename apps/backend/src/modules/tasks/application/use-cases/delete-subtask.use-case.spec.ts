...
import { DeleteSubtaskUseCase } from "./delete-subtask.use-case.js";
...
describe("DeleteSubtaskUseCase", () => {
  it("deletes a subtask belonging to the task", async () => {
    ...
    const useCase = new DeleteSubtaskUseCase(taskRepository, spaceRepository, subtaskRepository);
    ...
    expect(subtaskRepository.delete).toHaveBeenCalledWith("subtask-1");
  });
  ...
  it("throws NotFoundError when the parent task belongs to a different space", async () => {
    const taskRepository = buildTaskRepository({
